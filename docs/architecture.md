...
- **特徴**:
  ...
  - **ToDoとガントチャート**: 1つのポリシーに対して複数のToDo項目の登録が可能。各項目に「担当者（担当者未定含む）」「開始日」「期限」を設定し、UI上で自動的にガントチャート化して視覚的に進捗を管理できます。
...
#### 7.2 データモデル
- **Policy**: `title`, `description`, `priority`（`'high'` / `'medium'` / `'low'`）, `todos` (JSON形式によるタスク・担当・スケジュールの配列) を保持。

...
1. [主な機能](#主な機能)
   ...
   - [7. 政策リスト](#7-政策リスト)
     - [7.1 機能概要](#71-機能概要)
     - [7.2 データモデル](#72-データモデル)
   - [8. マルチテナント構成](#8-マルチテナント構成)
     - [8.1 権限モデル](#81-権限モデル)
     - [8.2 多重所属](#82-多重所属)
     - [8.3 組織切り替え](#83-組織切り替え)
     - [8.4 組織管理機能](#84-組織管理機能)
   - [9. ユーザー管理](#9-ユーザー管理)
     - [9.1 ユーザーCRUD](#91-ユーザーcrud)
     - [9.2 プロフィール設定](#92-プロフィール設定)
   - [10. 認証とセキュリティ](#10-認証とセキュリティ)
     - [10.1 認証機能](#101-認証機能)
     - [10.2 招待リンク機能](#102-招待リンク機能)
     - [10.3 パスワードリセット](#103-パスワードリセット)
   - [11. 通知システム](#11-通知システム)
...
### プライバシーと公開範囲
- 未公開のレポートは管理者のみが閲覧、検証可能
- 各「声（回答）」は常に匿名化（または特定個人の表示を除外）して分析
...
- **プロンプト調整**:
   1. **ファシリテート（議論の整理・進行）**:
      ...
      2. **ネクストアクション提案**:
         - 具体的なアクションプランの提示
         - To-Do、担当、スケジュールの枠組み整理
         - ※ただし、ユーザーに対し「誰が担当するか」「いつまでにやるか」を強要・質問することは避け、あくまでフレームワークの提案に留めるよう調整

### 7. 政策リスト
対話を通じて得られた課題解決のためのアイデアを、具体的な「政策（ポリシー）」として立案、管理し、実行に移すための機能です。

#### 7.1 機能概要
- **API**: `backend/api/dashboard.py` (ポリシーのCRUD)
  - `GET /api/dashboard/sessions/{session_id}/policies`
  - `POST /api/dashboard/sessions/{session_id}/policies`
  - `PUT /api/dashboard/policies/{policy_id}`
  - `DELETE /api/dashboard/policies/{policy_id}`
  - `POST /api/dashboard/policies/{policy_id}/evaluate` - 政策への投票登録/更新
- **UIコンポーネント**: 
  - `frontend/src/app/dashboard/sessions/[id]/page.tsx`
- **特徴**:
  - **ポリシー立案**: 課題チャットなどから得た「やるべきこと」を、ポリシーとその説明として記録。
  - **優先順位**: 各政策に「高（high）」「中（medium）」「低（low）」の優先順位を設定可能。UIでは色付きバッジ（高=赤・中=黄・低=青）で視覚的に表示される。
  - **対象グループ**: 政策がどのディスカッショングループに紐づくかを選択。グループ番号に応じたカラーバッジで表示される。「全体」を選択すると全グループ対象となる。
  - **ステータス管理**: 政策の進捗を「提案」→「可決」→「実行中」→「完了」の4段階で管理。UIでは色付きバッジ（提案=グレー・可決=セージ・実行中=ブルー・完了=グリーン）で視覚的に識別できる。
  - **表示順**: 政策リストはID降順（新しく立案した順）で表示され、編集しても位置が変わらない。
  - **ToDoとガントチャート**: 1つのポリシーに対して複数のToDo項目の登録が可能。各項目に「担当者（担当者未定含む）」「開始日」「期限」「ステータス（未着手/進行中/完了）」を設定し、UI上で自動的にガントチャート化して視覚的に進捗を管理できます。
  - **5段階の重みを持たせた投票**: 各政策に対してメンバーが1〜5の重み（賛同度・期待度）を持たせて投票でき、平均値と投票件数を表示。投票は1ユーザーにつき1件（更新可能）。
  - **インライン編集**: リストを展開した状態で、シームレスに説明文やToDoの追加・編集・削除が可能。
  - **FBアンケート作成**: 各政策の詳細ビューに「FBアンケートを作成」ボタン（組織管理者以上のみ表示）を設置。クリックすると`/dashboard?tab=surveys`へ遷移し、`SurveyManager`コンポーネントの新規作成フォームが自動開き、政策名と「本政策についてのフィードバックのご意見をください」が初期入力済み起動する。

#### 7.2 データモデル
- **Policy**: `title`, `description`, `priority`（`'high'` / `'medium'` / `'low'`）, `target_group`（対象ディスカッショングループ名、デフォルト`'全体'`）, `status`（`'提案'` / `'可決'` / `'実行中'` / `'完了'`）, `todos` (JSON形式によるタスク・担当・スケジュール・ステータスの配列) を保持。
- **PolicyEvaluation**: `policy_id`, `user_id`, `rating`（1〜5の整数）を保持。1ユーザー1政策につき1レコード（更新可能）。

### 8. マルチテナント構成
本システムは、1つのシステム内で複数の組織（部署、プロジェクト、案件など）を独立して管理できるマルチテナント構成を採用しています。
...
| **`report_published`** (AI分析更新) | AIスレッド分析実行・更新 | 組織全メンバー | 全員 | `/dashboard/sessions/{id}?title={議題}` | dashboard.py:601, 612 |
...
│   │   │   │   └── page.tsx    # ダッシュボードホーム（タブ式: データ分析/レポート/アンケート/フォーム作成管理など）

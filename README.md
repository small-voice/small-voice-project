...
- **グループごとの議論チャット**： メンバーをランダムに各グループに割り当て、各グループごとに専用のチャット空間を生成し、各課題に応じた議論を行う。
- **AIファシリテーター**： 各チャットの対話をAIが中立的なファシリテーターとして分析し、合意形成をサポート。
- **政策リスト**： 対話から生まれたアイデアを、具体的な政策として立案し、管理、実行に移します。
- **政策FB**： 立案した政策ごとに、フィードバック収集用のアンケートをワンクリックで作成。政策の実行結果を次の広聴サイクルへと繋げます。
